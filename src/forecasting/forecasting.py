...
def enable_dropout(model):
    """Enable dropout layers during inference to introduce randomness."""
    for module in model.modules():
        if isinstance(module, torch.nn.Dropout):
            module.train()  # Keep dropout active during inference
            logger.info(f"Activated dropout: {module}")  # Debugging


def forecast_one_step_lstm(
    model, data: pd.DataFrame, scaler, sequence_length: int = 120, horizon: int = 5
) -> (float, float):
    """
    ...
    recent_sequence = (
        data["Close"].tail(sequence_length).values.reshape(1, sequence_length, 1)
    )
    ...
    with torch.no_grad():
        predicted_return = model(
            torch.tensor(recent_sequence, dtype=torch.float32).to(
    ...
    #     predicted_return, -0.2, 0.2
    # )  # Limit returns to -20% to +20%
...
def forecast_multi_step(
    model, data: pd.DataFrame, horizon: int = 5, steps: int = 3
) -> list:
    """
    ...
    for step in range(steps):
        pred_return, pred_price = forecast_one_step(
            model, current_data, horizon=horizon
        )
        forecasts.append(
            {
                "step": step + 1,
